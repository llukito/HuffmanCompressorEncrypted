...
/* Constant: PSEUDO_EOF
 * A constant representing the PSEUDO_EOF marker that we will
 ...
 * actually hold a value.	 When we are constructing Huffman
 * encoding tree, we should set the characters in each internal
...
struct Node {
	 ...
	 * check whether or not you are leaking memory.
	 */
